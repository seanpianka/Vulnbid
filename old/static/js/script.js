...
( function($) {
	...
	$(window).load( function() {
		...
		var img = new Image();
		img.src = "/static/img/circuit-header-dark.png";
		...
		function setShowcaseCSS() {
			...
			if ( imgHeight < headerHeight + contentHeight ) {
				...
				if ( loadedImage ) {
					$headerWrap.css( 'background-image', 'url(' + img.src + ')' );
				} else {
					$headerWrap.css( 'background-image', 'url(' + $headerImg.attr( 'src' ) + ')' );
				}
				img.onload = function() {
					// Mobile instantly loads background image
					$headerWrap.css( 'background-image', 'url(' + img.src + ')' );
					loadedImage = true;
				}
				...
				img.onload = function() {
					// There should be better ways of doing this
					if ( document.getElementById("jqueryfade") ) {
						document.getElementById("jqueryfade").style.backgroundImage = 'url('+img.src+')';
					}
					jQuery("#header_img").animate({ opacity: 0 }, 'slow');
				}
